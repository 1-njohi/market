...
<template>
    ...
    <div>
        <!-- Header -->
        <div
            class="mb-6 flex flex-wrap items-center justify-between gap-4 border-b border-[#232d42] pb-6"
        >
            <div>
                <h1
                ...
                <p class="mt-1 text-xs text-slate-500">
                    Full ledger, filterable and exportable
                </p>
            </div>
            <button
                @click="exportCsv"
        ...
        <div class="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
            <div class="rounded-lg border border-[#232d42] bg-[#161c2a] p-4">
                ...
                <p class="mt-1 text-2xl font-black text-white">
                    {{ fmtCompact(counts.total_volume) }}
                </p>
                <p class="mt-0.5 text-[10px] text-slate-500">
                    {{ counts.total_count.toLocaleString() }} transactions
                </p>
            </div>
            <div class="rounded-lg border border-[#232d42] bg-[#161c2a] p-4">
                <span
                ...
                <p class="mt-1 text-2xl font-black text-emerald-400">
                    {{ fmtCompact(counts.credit_volume) }}
                </p>
                <p class="mt-0.5 text-[10px] text-slate-500">
                    {{ counts.credit_count.toLocaleString() }} entries
                </p>
            </div>
            <div class="rounded-lg border border-[#232d42] bg-[#161c2a] p-4">
                <span
                ...
                <p class="mt-1 text-2xl font-black text-rose-400">
                    {{ fmtCompact(counts.debit_volume) }}
                </p>
                <p class="mt-0.5 text-[10px] text-slate-500">
                    {{ counts.debit_count.toLocaleString() }} entries
                </p>
            </div>
            <div class="rounded-lg border border-[#232d42] bg-[#161c2a] p-4">
                <span
                ...
                <p
                    ...
                    :class="
                        counts.net >= 0 ? 'text-emerald-400' : 'text-rose-400'
                    "
                >
                    {{ counts.net >= 0 ? '+' : '−'
                    }}{{ fmtCompact(Math.abs(counts.net)) }}
                </p>
                <p class="mt-0.5 text-[10px] text-slate-500">
                    Credits − Debits
                </p>
        ...
        <!-- Direction tabs -->
        <div
            class="mb-5 flex flex-wrap items-center gap-1 border-b border-[#232d42]"
        >
            <button
                v-for="tab in tabs"
                :key="tab.value"
            ...
            <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-[#232d42]/60 bg-[#161c2a]">
                    ...
                    >
                        ...
                        <td class="px-4 py-2.5">
                            <span
                                class="font-mono text-[10px] text-sky-400 uppercase"
                                >{{ tx.type }}</span
                            >
                        </td>
                        <td
                            class="px-4 py-2.5 text-right font-mono font-bold"
                            :class="
                                Number(tx.amount) >= 0
                                    ? 'text-emerald-400'
                                    : 'text-rose-400'
                        ...
                        <td
                            class="px-4 py-2.5 text-right font-mono text-slate-400"
                        >
                            {{ Number(tx.balance_after).toFixed(2) }}
                        </td>
                        <td
                            class="px-4 py-2.5 font-mono text-[10px] text-slate-500"
                        >
                            {{ tx.reference }}
                        </td>
                    </tr>
        ...
        >
            <span class="text-slate-500"
                ...
                {{ transactions.last_page }}</span
            >
            <div class="flex gap-2">
                <a
                    v-if="transactions.prev_page_url"
                    :href="transactions.prev_page_url"
                    class="rounded border border-[#232d42] px-3 py-1.5 text-slate-300 hover:text-sky-400"
                    >← Prev</a
                >
                <a
